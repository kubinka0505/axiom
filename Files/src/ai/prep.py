...
def resample_file(src: str, dst: str, sr: int, sr_init: int = None) -> str | None:
	...
	if not sr_init:
		try:
			...
			logger.error(f"Failed to read {src} for sample rate detection: {e}", file = os.sys.stderr)
			return None
	...
	try:
		...
		logger.warning(f"PyDub failed for {src} with filter {filter_str}: {e}", file = os.sys.stderr)
...
def main():
	...
	if not src.is_dir():
		logger.error("Input directory does not exist or is not a directory.", file = os.sys.stderr)
		raise SystemExit(1)
	...
	for file in files:
		...
		try:
			...
			logger.warning(f"Skipping {file.name}: cannot read file - {e}")
			continue
	...
	try:
		for idx, (file, sr_init, target) in enumerate(tasks, start = 1):
			...
			if out_path.exists() and not args.overwrite:
				logger.warning(f"{out_name} already exists.")
			else:
				...
				if success:
					...
					logger.info(f"[{progress}] {out_name} @ {target} Hz" + " " * 10, end = "\r")
				else:
					logger.error(out_name)
	except KeyboardInterrupt:
		logger.info("Operation interrupted by user.")

	# save label
	json_path = out / "_labels.json"
	...
	logger.info(f"Wrote label JSON to {json_path}")
	logger.info(f"Total generated files: {len(mapping)}")
