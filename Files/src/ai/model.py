...
def parse_args():
	...
	if args.mode.lower() == "train":
		...
		if args.epochs is None or args.epoch <= 0:
...
def load_model(checkpoint_path: str, device: Union[str, torch.device] = "cpu") -> nn.Module:
	"""
...
def main():
	...
	if args.mode.lower() == "infer":
		if not args.model:
			raise RuntimeError("Need --model to do inference")

		loaded = torch.load(args.model, map_location = device, weights_only = True)

		# Detect architecture based on state_dict keys
		if any("conv1.weight" in k for k in loaded.keys()):
			network = CutoffNetV1 # looks like conv1.weight means v1
		else:
			network = CutoffNetV2 # llse assume v2

		model = network().to(device)
		model.load_state_dict(loaded) # load actual state dict

		est = predict_cutoff(args.input, model, device)
		print(est)
		return est
	else:
	...
	try:
		for epoch in range(1, args.epochs + 1):
			...
			if epoch == 1 or not epoch % args.eval_each_epoch:
				val_loss = eval_epoch(model, val_loader, device, f"{train_prefix} Evaluating")

			ckpt_name = f"axiom_cutoff_{epoch}e_{step}_{architecture}.pt"
