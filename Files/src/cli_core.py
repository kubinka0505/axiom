...
group_processing_sr.add_argument(
	...
	metavar = "int",
	default = -1,
	help = "Approximate frequency step in sample rate estimation.\nAlways truncated to (sample_rate / 800) - 1."
)
...
if not args.inputs[0]:
	...
	if not args.inputs:
		logger.error("No inputs selected. Exiting.")
		raise SystemExit(1)
...
for path in expanded:
	...
	if p.is_dir():
		...
		logger.error(f"Ignored invalid path: {path}")
...
if args.normalize is not None:
	...
	args.normalize = max(NORMALIZE_MINs, min(args.normalize, NORMALIZE_MAX))
